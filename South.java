...
public class South {
    ...
    private int getSouthSum(){
      ...
      int msgPsum = 0, msgNsum = 0;
        for(int i = 0; i<len;i++) {
        if (i % 2 == 0) {
            msgPsum = ((msgPsum + ((int) this.initText.charAt(i) - 65)) % 26 + 26) % 26;
        } else {
     ...
     public String getSouthDiffusion(int dIndex){
            ...
            int a2, x2;
            for (int j = 0; j < len; j++) {
                x2 = (int) this.initText.charAt(j) - 65;
    ...
    public String getLastOrbit(int dIndex){
        ...
        if(this.initText.length()%2==0) {
            ...
            lastOrbit = formatter.format(north.getFirstOrbit(dIndex));
        }else{
            SouthReversed southReversed = new SouthReversed(this.initText);
            lastOrbit = formatter.format(southReversed.getFirstOrbit(dIndex));
        }
